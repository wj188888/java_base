...
## GC回收
GC的作用区域：堆heap和方法区
> JVM在进行GC时，并不是这3个区域统一回收，大部分时候，回收都是新生代

- 新生代 （伊甸园区，幸存0区和幸存1区【相互切换】）
- 幸存区 （from丶to）
- 老年区

GC两种类：轻GC（普通的GC）丶重GC（全局GC）

题目：
- JVM的内存模型和分区，详细到每个分区是放什么的？
- 堆里面的分区有哪些？Eden，from,to,老年区，说说它们的特点！
- GC的算法有哪些？ 标记清除法，标记整理（压缩），复制算法，引用计数法，怎么用的？
- 轻GC和重GC分别在什么时候发生？

---
> 引用计数法

- 对每个对象给一个计算器，被引用后+1,计数器会被消耗；（被淘汰了）

> 标记复制法（好处：好处没有内存的碎片）（缺点:浪费内存空间，多了一半空间永远是空to）

- 谁空谁是to
- 每次GC 都会讲Eden活的对象移到幸存区中，一旦Eden区被GC后，就会是空的！
- 当一个对象经历了15次GC，都还没有死， -XX：MaxTenuringThreshold=5,通过这个参数可以设定进入老年代的时间
- 主要是复制算法

复制算法最佳使用场景：对象存货度较低的时候：最要是用于新生区；
...
一天时间学JVM，需要花时间，多看面试题，以及《深入理解JVM》虚拟机
但是可以，掌握学习方法
...
1.什么是JMM？
答：java内存模型》java memory model
2.它是干嘛的？
答：官方，其他人的博客，对应的视频！
作用：缓存一致性协议，用于定义数据读写的规则（遵守，找到这个规则）
抛出问题：分为主内存和线程内存，每个线程都有做一个自己的工作区域，那么主内存和线程内存可能数据不一致；
JMM定义：线程工作内存和主内存是抽象 关系，线程之间的共享变量存储在主内存中，每个线程都有一个本地私有的内存；
...
面试：
10题，只过了三个；pass ， 面经10个，， 分析不仅仅者这10个，看看周围的问题是否也会有关联？找百度面试题？
通过大量的面试总结，得出一套解决的思路？3-4套；